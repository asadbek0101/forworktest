    ...
    <div id="first-test-page">
      ...
      <div v-if="show" class="test">
        ...
        <ul>
          ...
          <MyListActive v-for="(item, index) in 10" :key="index" :index="index" @newactive="indexActive = $event, count=$event" :active="index === indexActive" >{{item}}</MyListActive>
      ...
      <div v-else class="result">
            <div v-for="(item, index) in resultArray" :key="index" class="result-item">
                ...
                <p v-for="(answer, i) in item.answers" :key="i" :style="{backgroundColor: (answer.sub && answer.target)?'#F7D716':(answer.sub)?'green':(answer.target)?'red':''}">
                  {{i==0?'A':i==1?'B':i==2?'C':'D'}}) {{answer.title}} <font-awesome-icon v-if="answer.sub" icon="fa-solid fa-check" /> <font-awesome-icon v-if="(!answer.sub && answer.target)" icon="fa-solid fa-xmark" />
                </p>
            </div>
          <div class="d-flex">
              <p>Natija : {{result}}/100</p> <button @click="show = true; result = 0">OK</button>
          </div>
      </div>
...
export default {
  ...
  methods:{
    ...
    targetAnswer(element){
          this.data[this.count].answers.map((t)=>{
            t.target = false
          })
          element.target = true
  ...
  computed:{
    data(){
      return this.$store.state.firstTestsGroup
    }
  }
}
</script>
